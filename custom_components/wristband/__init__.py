...
async def async_setup(hass, config):
    ...
    #     time.sleep(5)
    #     list = findingdevice(config[DOMAIN])
    for item in config_device:
        print(item)

    for i in range(len(config_device)):
        print('\033[1;32m',i + 1, config_device[i],'\033[0m')
        thread = threading.Thread(target=WristbandState, args=(hass, config_device[i]))
        thread.setName(config_device[i])
        thread.setDaemon(True)
        threadList.append(thread)
        thread.start()

    def async_wristband_multiservice(service):
        _LOGGER.info("多线程服务被调用")
        params = service.data.copy()
        params.pop(ATTR_ENTITY_ID, None)

        for item in threadList:
            print(params["macAddress"] + "=======" + item.getName())

    hass.services.async_register(DOMAIN, 'async_wristband_multiservice', async_wristband_multiservice)
...
class WristbandState(object):
    ...
    def __init__(self,hass, attr):
        ...
        # self._step = step
        self._attr = attr
        self._mac = self._attr
        self._state = 0
        self.peri = bluepy.btle.Peripheral()
        self.peri.setDelegate(MyNotifyDelegate())
        # 在类初始化的时候，设置初始状态,将手环数据在state中显示出来
        #
        # self._hass.states.async_set(DOMAIN + "." + re.sub('\s+', '', self._attr).replace(" ", "_"),
        #                             re.sub('\s+', '', self._attr).replace(" ", "_"),
        ...
        while True:
            print('\033[1;31m try to connect --',self._mac,'\033[0m')
            try:
                self.peri.setDelegate(MyNotifyDelegate())
                self.peri.connect(self._mac, bluepy.btle.ADDR_TYPE_RANDOM)
            except:
                time.sleep(3)
                continue
            ...
            print('\033[1;32m connected -----', self._mac, ' \033[0m')
            self.peri.writeCharacteristic(HANDLE_NOTIFY, b'\x02\x00')
            self.peri.writeCharacteristic(HANDLE_NOTIFY_2, b'\x02\x00')
            self.peri.writeCharacteristic(HANDLE_NOTIFY_3, b'\x02\x00')
    ...
    def update(self,now):
        ...
        #设置新的状态值
        # self._hass.states.async_set(DOMAIN, attributes=self._attr)
